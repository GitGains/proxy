...
gg-save_proxy_variables() {
  ...
  if command -v npm &> /dev/null; then
    echo "ORIGINAL_NPM_CONFIG_REGISTRY=$(npm config get registry)" >> "$GG_PROXY_CONFIG"
  fi

  # Save the current yarn registry configuration if yarn is installed
  if command -v yarn &> /dev/null; then
    echo "ORIGINAL_YARN_CONFIG_REGISTRY=$(yarn config get registry)" >> "$GG_PROXY_CONFIG"
  fi

  # Save the current pip index URL configuration if pip is installed
  if command -v pip &> /dev/null; then
    echo "ORIGINAL_PIP_CONFIG_INDEX_URL=$(pip config get global.index-url)" >> "$GG_PROXY_CONFIG"
  fi
  # Save the current pip3 index URL configuration if pip3 is installed
  if command -v pip3 &> /dev/null; then
    echo "ORIGINAL_PIP3_CONFIG_INDEX_URL=$(pip3 config get global.index-url)" >> "$GG_PROXY_CONFIG"
  fi
  # Save the current python pip index URL configuration if python is installed
  if command -v python &> /dev/null; then
    echo "ORIGINAL_PYTHON_CONFIG_INDEX_URL=$(python -m pip config get global.index-url)" >> "$GG_PROXY_CONFIG"
  fi
  # Save the current python3 pip index URL configuration if python is installed
  if command -v python3 &> /dev/null; then
    echo "ORIGINAL_PYTHON3_CONFIG_INDEX_URL=$(python3 -m pip config get global.index-url)" >> "$GG_PROXY_CONFIG"
  fi

  # Save the current GOPROXY configuration if Go is installed
  if command -v go &> /dev/null; then
    echo "ORIGINAL_GOPROXY=$(go env GOPROXY)" >> "$GG_PROXY_CONFIG"
  fi
}
...
gg-activate() {
  ...
  if command -v go &> /dev/null; then
    if [ -z "$GOPROXY" ]; then
      ...
      go env -w GOPROXY="$GG_PROXY_URL,$(go env GOPROXY)" &> /dev/null
    fi
  fi

  # Set npm config if npm is installed
  if command -v npm &> /dev/null; then
    npm config set registry "$GG_PROXY_URL"
  fi
